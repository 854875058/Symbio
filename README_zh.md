...
Web UI 共 18 个页面，默认暖色浅底主题（Claude / Hermes 风格，可一键切换深色）。
...
<tr>
<td width="50%"><b>个人微信双向机器人 · 扫码绑定</b><br/><img src="https://raw.githubusercontent.com/854875058/Symbio/master/assets/screenshots/ui-wechat.png" alt="WeChat Bridge"></td>
<td width="50%"><b>本体记忆图谱</b><br/><img src="https://raw.githubusercontent.com/854875058/Symbio/master/assets/screenshots/ui-ontology.png" alt="Ontology Graph"></td>
</tr>
</table>
...
| HITL + IM 多渠道审批 | ✅ 已实现 | 微信/QQ/飞书/钉钉/Telegram/Slack + 超时升级策略（自动拒绝/通过/转交管理员） |
...
当前 Web UI 覆盖 18 个页面，采用左侧分组侧边栏布局，默认暖色浅底主题（Claude / Hermes 风格，可一键切换深色）：

| 分组 | 页面 | 说明 |
|------|------|------|
| 核心 | 对话 | 多会话、流式回答、模型选择、历史持久化 |
| 智能体 | 任务 | 任务列表、DAG 可视化、执行事件 |
| 智能体 | 审批 | HITL 审批列表、渠道配置、超时升级策略 |
| 智能体 | Agent 工作台 | 单 Agent 调试台：提示词、工具、单步执行 |
| 智能体 | A2A | AgentCard 展示、出站会话、入站任务 |
| 智能体 | 微信机器人 | 扫码绑定、消息路由、实时消息流 |
| 智能体 | 外部 Agent | Codex/Claude Code 会话接管、接力模式、transcript 导入 |
| 工具 | Skills | 本地 Skill 管理、市场浏览、文件编辑 |
| 工具 | Sandbox | 命令执行、权限策略、审批策略、审计 |
| 工具 | MCP | MCP server 管理、工具列表探测 |
| 工具 | 安全 | 三层防火墙概览、威胁分布、在线扫描、红队自检、审计 |
| 工具 | Computer Use | 浏览器会话、目标驱动规划、动作审计时间线、回放 |
| 知识 | 记忆 | 语义搜索、按重要度筛选、写入、统计 |
| 知识 | 本体图谱 | 实体关系可视化 |
| 分析 | 仪表盘 | Token 趋势图、可观测摘要、成本中心（用量/缓存命中率/预算） |
| 分析 | 能力账本 | 承诺能力与实现状态对照 |
| 分析 | 数据飞轮 | 四阶段闭环、失效分析、SOP、Dataset 导出、Eval 解析 |
| 配置 | 模型 | 模型池配置、API Key 管理、连接测试、人工审批策略 |

每个页面的"是什么 / 为什么存在 / 怎么用"见 [页面手册](docs/pages.md)；视觉规则见 [DESIGN.md](DESIGN.md)。
