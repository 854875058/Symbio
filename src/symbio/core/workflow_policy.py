...
class WorkflowPolicy(BaseModel):
    ...
    def to_prompt(self) -> str:
        ...
        if self.require_plan:
            lines.append(
                "- Start by producing a brief implementation plan before changing behavior."
            )
        if self.require_tdd:
            lines.extend(
                [
        ...
        if self.require_root_cause_before_fix:
            lines.append(
                "- For bugs or test failures, investigate and state the root cause before proposing or applying a fix."
            )
        if self.require_spec_review:
            lines.append(
                "- Check the final result against the requested scope; avoid unrequested extra behavior."
            )
        if self.require_verification_before_completion:
            lines.append(
                "- Before completion, run or cite the exact verification that proves the result."
            )
        if not self.allow_assumptions:
            lines.append(
                "- Do not silently invent missing constraints; ask or mark the uncertainty explicitly."
            )
        if self.checklist:
...
def workflow_policy_for_intent(intent: Intent) -> WorkflowPolicy:
    ...
    is_feature_work = is_code_work or any(
        keyword in text
        for keyword in [
            "implement",
            "add",
    ...
    if is_feature_work or is_bug_work:
        checklist.append(
            "Run targeted verification, then broader verification if the change touches shared behavior."
        )
