...
class EntityExtractor:
    ...
    _STOP_WORDS: set[str] = {
        ...
        "Which", "While", "With", "From", "Have", "Has", "Had", "Been",
        "Being", "Would", "Could", "Should", "Will", "Can", "May", "Might",
        "Must", "Shall", "Not", "But", "And", "For", "Nor", "Yet", "So",
        "Are", "Was", "Were", "Did", "Does", "Done", "Doesn", "Didn",
        "Isn", "Aren", "Wasn", "Weren", "Hasn", "Haven", "Hadn",
        "Each", "Every", "Some", "Any", "None", "All", "Both",
        "Most", "Many", "Few", "Several", "Other", "Another",
        "However", "Therefore", "Furthermore", "Moreover", "Otherwise",
        "Instead", "Finally", "First", "Second", "Third",
        "New", "Old", "Big", "Small", "Good", "Bad", "High", "Low",
        "Long", "Short", "Fast", "Slow", "Hard", "Easy", "True", "False",
        "Note", "See", "Also", "Just", "Only", "Even", "Still", "Already",
        "Here", "There", "Now", "Then", "After", "Before", "Since",
        "Until", "Above", "Below", "Between", "Under", "Over",
        "Let", "Set", "Get", "Run", "Use", "Make", "Take", "Give",
        "Put", "Keep", "Try", "Call", "Ask", "Tell", "Show", "Work",
        "Need", "Want", "Like", "Look", "Find", "Think", "Know",
        "Come", "Go", "See", "Take", "Make", "Say", "Back",
    }
...
class RelationExtractor:
    ...
    _PATTERNS: list[tuple[str, str, float]] = [
        ...
        (r"(\w[\w\s]*?)\s+(?:depends?\s+on|requires?)\s+(\w[\w\s]*?)(?:[,.]|$)", "depends_on", 0.85),
...
class AutoPopulator:
    ...
    async def _store_entity(
        ...
        if concept_id and concept_id not in [
            c for c in self.ontology._concepts
        ]:
            # 如果是新概念，记录到结果中
    ...
    def _ensure_concept(self, entity_type: str) -> Optional[str]:
        ...
        description = self._concept_descriptions.get(concept_name, f"自动创建的 {concept_name} 概念")
