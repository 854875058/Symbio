...
**配套工具：dataset_exporter.py — 微调语料清洗与导出管道**

专门处理数据飞轮的"最后一公里"，将脏的 Trace 轨迹转换成高质量的问答数据集：
- **多格式支持** — ShareGPT、Alpaca、OpenAI 等主流微调数据格式
- **自动清洗** — 去重、去噪、格式标准化
- **质量过滤** — 基于长度、完整性、语义质量评分筛选高质量数据
- **PII 脱敏** — 自动检测并移除敏感信息（邮箱、手机号、密钥等）
- **增量导出** — 只导出新增的高质量数据，避免重复处理
...
**配套工具：test_runner.py — 测试框架驱动器**

专门负责挂载测试用例并解析 stderr/stdout，直接将执行结果作为强约束反馈给 DAG 引擎：
- 支持 pytest、npm test、jest 等主流测试框架
- 标准化测试结果输出（Pass/Fail/Error + 详细日志）
- 沙箱化执行，防止测试代码影响主进程
- 超时控制，防止测试卡死
